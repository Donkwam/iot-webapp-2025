...
4. MySQL 설치
    ...
    3. 일반적으로 Next
    4. Authentication Method > Use Legaucy Authentication (Retain MySQL 5.x Compatibility) 선택
        - 암호정책이 간결
        - 대신 AWS는 IP나 공개된 상황이라 간단한 암호하면 절대 안됨
    5. 나머지는 Next, Execute 실행
    6. 마지막에 Finish 클릭
    7. Firewall & Network Protection 실행 > Advanced setting 선택
        - Inbound Rules > Port 3306확인, 없으면 생성
...
5. FileZilla FTP 서버 설치
    ...
    4. 프로토콜 셋팅 > FTP and FTP over TLS메뉴
        - Connection Secuity
            - Fenerate new 버튼 클릭 후 OK
        - Passive Mode

...
namespace MyPortfolioWebApp.Controllers
{
    public class NewsController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (UploadFile != null && UploadFile.Length > 0) {
                    ...
                    if (UploadFile.Length > MaxFileSize) {
                        ModelState.AddModelError("UploadFile", "파일크기는 10MB로 제한합니다.");
                        return View(news);
                    }
                    ...
                    // example.jpg 파일이 여러번올라가면 파일이 겹쳐짐
                    // 파일명을 변경
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, News news, IFormFile? NewFile)
        {
            if (id != news.Id)
            {
                return NotFound();
